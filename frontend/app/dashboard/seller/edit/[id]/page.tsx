...
export function EditTourForm({ tourId, onSuccess }: { tourId: string | number; onSuccess?: any }) {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          setForm({
            ...
            booking_deadline_days: tour.booking_deadline_days || 3,
            ...
            fitness_requirement: tour.fitness_requirement || "low",
          });
  ...
  // Save tour function (can be called from any step)
  const saveTour = async (showSuccessMessage = true) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const errorMsg = res.error || res.message || "Lỗi khi cập nhật tour";
        setToast({ message: errorMsg, type: "error" });
        console.error("Update tour error:", res);
        return false;
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    // Final submit - redirect after success
    await saveTour(true);
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-blue-50 py-8 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-blue-50 py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Sửa Tour</h1>
          <p className="text-gray-600">Chỉnh sửa thông tin tour của bạn</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-8 border border-gray-100">
          ...
          <p className="text-xs text-gray-500 text-center mt-4">
            💡 Click vào bất kỳ bước nào để điều hướng nhanh
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between mt-8 gap-4">
            ...
            <div className="flex gap-3">
              {/* Save Draft Button - Available on all steps except last */}
              {currentStep < 6 && (
                <button
                  type="button"
                  onClick={handleSaveDraft}
                  disabled={saving || uploading}
                  className="flex items-center gap-2 px-5 py-3 bg-gray-100 text-gray-700 rounded-xl font-semibold hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all border-2 border-gray-300"
                >
...
export default function EditTourPage({ params }: { params: { id: string } }) {
  return <EditTourForm tourId={params?.id} />;
}
